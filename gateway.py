import json
import build
from compile import SHA3_256, REGISTRY, HYCACHE
...
def start_container(id: str, use_other_ports=None):
    ...
    return subprocess.check_output(command +' --detach '+id, shell=True).decode('utf-8').replace('\n', '')
...
if __name__ == "__main__":
   ...
   class Gateway(gateway_pb2_grpc.Gateway):
    ...
    # ¡¡ Empyy protocol_mesh and application_def !!
    GATEWAY_INSTANCE.api.append(slot)
    uri = gateway_pb2.ipss__pb2.Instance.Uri()
    uri.ip = '172.17.0.1'
    uri.port = GATEWAY_PORT
    GATEWAY_INSTANCE.uri_slot[GATEWAY_PORT] = uri
