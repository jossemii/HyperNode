...
from tests.main import RANDOM, FRONTIER, GATEWAY, generator as gen
...
def test_frontier_solver():
    ...
    uri = frontier.instance.uri_slot[0].uri[0]
    r_uri = uri.ip + ':' + str(uri.port)

    r_stub = api_pb2_grpc.SolverStub(
        grpc.insecure_channel(r_uri)
    )

    while True:
        sleep(5)
        print(f"Trying to resolve.")
        try:
