...
def prevent_ram_kill(acumulator: int = 0) -> tuple:
    ...
    while True:
        ...
        if used_ram > HIGHT_RAM_MARGIN or acumulator >= 0 and randint(0, 100 - int(acumulator)) < used_ram or choice([True, False], 1, p=[abs(int(acumulator*0.01)), 1-abs(int(acumulator*0.01))])[0]:
            print('wait for more RAM. ', acumulator)
            acumulator += used_ram*0.1 if prev_mem - used_ram < 0 else (used_ram - prev_mem)
            prev_mem = used_ram
            sleep(used_ram*0.01)
        else:
            print('                 yield ', acumulator)
            return acumulator-0.01
